...
class BurpExtender(_BurpExtenderBase):
    ...
    def _build_agent_bootstrap_prompt(self, url, token):
        ...
        return (
            ...
            "  Kali Linux:\n"
            "    ss -ltn | grep ':8080 '  # Burp must be listening first\n"
            ...
            "\n"
            "=== BROWSER VERIFICATION (browser_verify=TRUE) ===\n"
            "Use BrowserOS MCP only; no curl unless BrowserOS fails. Re-snapshot after navigation; use screenshot/DOM/console logs as evidence. Ask before state-changing actions, persistent payloads, or out-of-scope actions. If BrowserOS fails, note 'browser verify skipped' and fall back to proxied curl only.\n"
            "\n"
            "=== TESTING METHODOLOGY ===\n"
            "0. PRIOR WORK GATE: Already tested this endpoint/technique? Report 'already covered' and skip.\n"
            "1. Prioritize authz/IDOR, injection, SSRF/XXE, XSS, logic/race, session/auth, sensitive data exposure, and meaningful chains based on the queued evidence.\n"
            "2. Follow active_test_recipe first. Keep requests minimal; stop when signal is low.\n"
            "3. For blind/OOB cases use Collaborator; for HTTP/2-sensitive cases use /api/agent/request/http2.\n"
            ...
            "Do not report standalone: missing headers, self-XSS/no delivery, logout/login CSRF without impact, no-rate-limit without abuse path, fixed-origin CORS, non-sensitive clickjacking, banner/version disclosure without exploitable CVE, non-session cookie flags, tabnabbing without sensitive context, host-header injection without impact, untriggered races, or outdated libraries without exploitable code path. Mark as not_important/false_positive and move on.\n"
            "\n"
